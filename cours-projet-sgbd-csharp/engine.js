/* ===== Moteur du cours projet-sgbd-csharp — rend la page à partir de l'objet DAY ===== */
(function () {
  "use strict";
  ...
  document.title = "Jour " + DAY.num + " — " + DAY.title;

  var header = el("header", "day-header",
    '<div class="kicker">Projet de développement SGBD (C#) — 31 jours</div>' +
    "<h1>Jour " + DAY.num + " · " + DAY.title + "</h1>" +
    '<p class="subtitle">' + (DAY.subtitle || "") + "</p>");
  wrap.appendChild(header);

  var nav = el("nav", "daynav");
  var prev = DAY.num > 1 ? '<a href="jour' + pad(DAY.num - 1) + '.html">← Jour ' + (DAY.num - 1) + "</a>" : "<span></span>";
  var next = DAY.num < 31 ? '<a href="jour' + pad(DAY.num + 1) + '.html">Jour ' + (DAY.num + 1) + " →</a>" : "<span></span>";
  ...
  wrap.appendChild(el("footer", "pagefoot", "Jour " + DAY.num + " / 31 — Refais les exercices jusqu'à obtenir un score parfait : c'est la répétition qui crée l'automatisme."));
  ...
  function balanceOpts(ex) {
    ...
    var tails = [
      ...
      ", ce qui ralentirait inutilement l'algorithme",
      ", sauf si l'énoncé précise le contraire",
  ...
  function updateScore() {
    ...
    try {
      var key = "projsgbdcs31-j" + DAY.num;
      var prev = parseInt(localStorage.getItem(key) || "0", 10);
      if (pts > prev) localStorage.setItem(key, String(pts));
      localStorage.setItem("projsgbdcs31-total-j" + DAY.num, String(totalQuestions));
