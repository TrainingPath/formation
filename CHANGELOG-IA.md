...
## cours-java — leçon 29 : threads et concurrence (lot du 6 août 2026)

**Contrainte technique traitée de front.** Un corrigé sur les fils d'exécution ne peut pas être vérifié comme les autres : l'ordre d'affichage varie d'une exécution à l'autre, et le vérificateur exige des sorties reproductibles. Les trois corrigés sont donc conçus pour que **tout ce qui est affiché soit déterministe** — chaque fil écrit dans sa propre case, et le fil principal n'affiche qu'après les avoir tous rejoints. La seule valeur non reproductible du lot (le nombre d'incréments perdus par un compteur non protégé) est **affichée sans être testée**, et la remarque de 29.1 explique pourquoi : « il n'est pas reproductible, donc aucun test ne peut l'exiger, donc il traverse les campagnes d'essais sans se montrer ». C'est le prolongement direct de la leçon 28.

**Contenu ajouté.** 3 exercices, 15 indices, 3 corrigés exécutés et certifiés. **Nouvelle mini-série** : « L'observatoire ornithologique », jours 29 à 31.

- **29.1 « La blanchisserie »** (niveau 1, domaine *blanchisserie*) — un fil, `start()` puis `join()` ; la même tâche par `run()` ; quatre machines écrivant chacune dans sa case (aucun verrou, résultat parfaitement reproductible : `Minutes machine cumulées : 249`) ; puis trois compteurs — nu, sous verrou, `AtomicInteger` — incrémentés 100 000 fois par quatre fils. Les deux compteurs protégés valent 100000 à tous les coups ; le nu a perdu 532 puis 113 incréments sur deux exécutions consécutives. L'indice ④ porte sur la contrainte du langage — une lambda ne capture qu'une variable définitive — et explique **pourquoi** elle existe. La note de fin creuse la vraie nuance : le tableau des durées **est** partagé et ne pose aucun problème ; le danger n'est pas l'objet partagé, c'est l'écriture simultanée au même endroit.
- **29.2 « Le centre d'appels »** (niveau 2, domaine *centre d'appels*, sous-questions a/b/c/d) — quarante appels, quatre opérateurs, **trois stratégies d'addition menées en parallèle** dans la même boucle. Résultat certifié et pédagogiquement plus fort que prévu : sur quarante additions, le total **non protégé est juste** — le programme affiche « juste cette fois-ci » et non « juste ». La remarque en fait le point central : « un défaut qui ne se manifeste pas est un défaut qu'on ne corrige pas », et la question de réflexion renvoie explicitement à la leçon 28 (quel test pourrait attraper une compétition de données — et lequel n'y arrivera jamais). Le tri des numéros donne `04100 … 0499`, ce qui fournit une sous-question sur les données mal typées. La sous-question (d) demande pourquoi le bilan par catégorie n'a **pas** été parallélisé : « le parallélisme se justifie quand la tâche est longue et découpable, pas parce qu'on vient d'apprendre à le faire ».
- **29.3 « L'observatoire ornithologique — 1/3 »** (niveau 3, ouverture de série, domaine *observatoire ornithologique*) — 48 relevés, une fonction de dépouillement écrite **une seule fois** et appelée de deux façons : séquentiellement (la référence) puis par quatre fils écrivant dans quatre tableaux disjoints. Le programme affiche les deux dépouillements côte à côte et **prouve** leur concordance. Le « pourquoi » nomme le réflexe désormais installé : leçon 25 (deux écritures du règlement), leçon 27 (deux assemblages du sommaire), leçon 29 (deux dépouillements) — trois contextes, un seul geste. Valeurs certifiées : MILAN 45, BUSE 46, FAUCON 48, EPERVIER 50, total 189, plus gros relevé 7 individus au poste FAL-03.

**Anti-fuite.** 9 violations dans les indices ⑤, corrigées par masquage (dont une qui a résisté à un premier essai : couper la ligne par des balises ne trompe pas le vérificateur, qui retire le balisage avant de comparer — il a fallu masquer un identifiant).
...
## cours-csharp, lot 2 — leçons 5 à 8 · mini-série « La patinoire municipale » (4/4)

12 exercices, 60 indices, 12 corrigés. Toujours sans champ `tests` : aucun compilateur C# n'est atteignable (voir la décision préalable du lot 1). `cslint.py` passe sur les 12 fichiers ; toutes les valeurs annoncées ont été recalculées en Python avant d'être écrites.

- **Leçon 5 — conditions.** 5.1 *La bibliothèque de quartier* (bibliothèque de quartier) : seuils enchaînés du plus haut au plus bas, `switch` expression, plafond d'amende traité en **deux instructions séparées** plutôt qu'en formule ingénieuse, `ToLower()` pour la comparaison, ternaire, `??`. Le même jour de la semaine est nommé **deux fois** — une fois par `switch` classique avec ses `break`, une fois par `switch` expression avec un motif `2 or 3 or 4 or 5` — pour faire voir en une lecture pourquoi le second a remplacé le premier partout où l'on produit une valeur. 5.2 *Le péage d'autoroute* (péage d'autoroute, a/b/c/d) : cinq décisions indépendantes combinées, montants calculés étape par étape et **tous affichés** — « un calcul dont chaque étape est visible peut être contesté par l'usager et vérifié par l'exploitant ; c'est la différence entre un ticket et une sommation ». La remarque assume la troncature des pourcentages en entiers : moins d'un centime par passage, plusieurs dizaines d'euros par jour.
- **Leçon 6 — boucles.** 6.1 *La salle de sport* (salle de sport) : les quatre boucles, chacune choisie pour la question qu'elle résout, dont une boucle conditionnelle qui fait **zéro tour** et l'affiche — « le jour où une boucle ne produira rien, tu ne perdras pas dix minutes à chercher pourquoi elle ne s'exécute pas ». 6.2 *La chaîne de conditionnement* (chaîne de conditionnement, a/b/c/d) : boucle de saisie qui s'arrête sur un **mot** et jamais sur un nombre, trois refus traités par abandon de tour plutôt que par imbrication, et surtout le **traitement du dernier groupe** — le carton en cours qu'il faut fermer après la boucle, « le cas de fin le plus souvent oublié de tout le cours ».
- **Leçon 7 — tableaux.** 7.1 *Les relevés de pluviométrie* (pluviométrie) : création avec valeurs contre création vide, parcours par indices contre parcours sans indice, champion provisoire gardant **valeur et position**, tableau de cumul dont chaque case lit la précédente — d'où un cas de départ à traiter hors boucle. 7.2 *Le tournoi de badminton* (tournoi de badminton, a/b/c/d) : cinq structures parallèles à tenir alignées à la main, grille carrée des rencontres, et la distinction délicate entre un match nul et une rencontre qui n'a pas eu lieu. La note de fin nomme la gêne et annonce qu'elle est le sujet de la leçon 12.
- **Leçon 8 — méthodes.** 8.1 *L'atelier de sérigraphie* (sérigraphie) : barème dans une seule méthode, paramètre par défaut, surcharge, et la démonstration du **passage par valeur** — une méthode qui prétend consommer du stock et ne consomme rien, « du code parfaitement légal, qui compile sans le moindre avertissement et ne fait rigoureusement rien ». 8.2 *La laverie automatique* (laverie automatique, a/b/c/d) : le **même programme écrit deux fois**, en vrac puis découpé, les deux appelés à la suite pour comparer les sorties. L'indice ④ pose la règle : « on refond, puis on améliore — jamais les deux à la fois ».
- **Série « La patinoire municipale », 1/4 à 4/4** : jour 1, un seul patineur et un tarif décidé par conditions ; jour 2, la file entière avec compteurs et accumulateurs — et la découverte que le compteur de places et le compteur de patineurs cessent d'être le même nombre dès qu'un enfant de moins de quatre ans entre gratuitement ; jour 3, la semaine rangée dans quatre tableaux alignés, un cinquième tableau **calculé**, un histogramme et une grille de créneaux ; jour 4, tout est rangé en quatorze méthodes et le programme principal ne fait plus que déclarer les données et appeler — **sans qu'un seul chiffre change**.

**Deux points de méthode installés par la série**, et nommés explicitement à l'élève : une refonte réussie ne se voit pas à l'écran (jour 4 : « 32 entrées, 262,00 €, 21 paires — vérifie les trois »), et un programme peut vérifier lui-même ce qu'il devrait trouver — le jour 4 se termine sur un contrôle affichant que la somme des créneaux redonne les entrées du jeudi, première vérification automatique du cours.

**Correction apportée en cours d'écriture.** La remarque de 6.1 affirmait d'abord que la division entière faussait la moyenne des minutes ; recalcul fait, 252 divisé par 7 tombe exactement sur 36 et rien n'est perdu. La remarque a été réécrite pour dire ce qui est vrai — et pour en tirer l'avertissement qui compte : « un essai réussi sur des données qui tombent rondes ne prouve strictement rien sur celles qui ne tomberont pas rondes ».

**Outillage.** `cslint.py` a appris deux formes qu'il refusait à tort : les lignes de continuation commençant par un opérateur, et les expressions `switch` (tête `= x switch` suivie de son bloc, dernière branche sans virgule finale).

**Vérification.** `node _verify_entrainement.js` : ✅ 80 leçons équipées, 240 exercices, 0 problème. Contrôle sur la machine après déploiement : 8 leçons, 24 exercices, 5 indices chacun, aucun champ `tests`.

**État du cours.** cours-csharp : 8 leçons sur 31 équipées, 24 exercices, 2 mini-séries terminées. Prochain lot : leçons 9 à 12 (string, `List<T>`, algorithmes traduits, POO).

---

## cours-csharp, lot 3 — leçons 9 à 12 · mini-série « Le club de Scrabble » (4/4)

12 exercices, 60 indices, 12 corrigés. Toujours sans champ `tests` (voir la décision préalable du lot 1). `cslint.py` : 12 fichiers sans anomalie ; toutes les valeurs annoncées recalculées en Python.

- **Leçon 9 — la classe string.** 9.1 *Le guichet de l'état civil* (état civil) : l'immuabilité démontrée par un nettoyage **non affecté**, les accès par crochets, `Substring` dont le second argument est une **longueur** et non une borne (différence avec Java), `Split`/`Join`, compteur de caractères, palindrome. Le découpage rend cinq morceaux et non quatre — deux espaces consécutifs encadrent un morceau vide, et la remarque le fait compter plutôt que de le taire. 9.2 *Les codes-barres du supermarché* (codes-barres, a/b/c/d) : validation caractère par caractère **sans expression régulière**, avec un diagnostic qui nomme la position fautive. Point de méthode : on **normalise avant de juger** — la moitié des codes acceptés ne l'ont été que grâce au nettoyage préalable, et le programme compte ce chiffre exprès, parce qu'il mesure l'écart entre ce que les gens tapent et ce que le format exige.
- **Leçon 10 — `List<T>`.** 10.1 *L'étal de la brocante* (brocante) : liste qui grandit, `Insert`, retrait par indice contre retrait par valeur, et le piège du **retrait en cours de parcours** démontré sur une liste séparée des données de l'exercice. 10.2 *La salle d'attente* (salle d'attente, a/b/c/d) : trois listes parallèles, une méthode d'inscription qui les touche ensemble, insertion en tête d'un cas urgent, et une boucle de réorientation qui retire de trois listes à la fois — l'indice ④ impose l'ordre : afficher, puis retirer, puis reculer.
- **Leçon 11 — algorithmes traduits.** 11.1 *Le podium d'athlétisme* (athlétisme) : recherche séquentielle, positions des extrêmes, copie explicite, tri à bulles, dichotomie — et la dichotomie appliquée **exprès à un tableau non trié**, dont la réponse fausse est affichée à côté de la réponse juste. « L'algorithme le plus dangereux du cours » : il ne se plaint jamais. 11.2 *L'annuaire téléphonique* (annuaire téléphonique, a/b/c/d) : échange à **trois tableaux**, deux tris successifs dont le second détruit l'ordre du premier, comparaison de textes écrite à la main avec le cas « un texte est le début de l'autre ». La note explique la **stabilité** du tri à bulles, qui laisse Bertrand avant Dumont à égalité d'appels.
- **Leçon 12 — POO.** 12.1 *La cave à vins* (cave à vins) : classe, exemplaires, méthodes sans `static`, liste d'objets, et la démonstration de l'**alias** — deux variables, un seul objet. 12.2 *Le parc de trottinettes* (parc de trottinettes, a/b/c/d) : quatre tableaux parallèles remplacés par une classe, recherche qui **rend l'objet** (donc permet de le modifier), `?.` sur une référence vide, et un tri dont l'échange tient en **trois lignes au lieu de neuf**.
- **Série « Le club de Scrabble », 1/4 à 4/4** : jour 1, un mot n'est qu'un texte — refus, barème, palindrome ; jour 2, les coups s'accumulent dans trois listes et la liste devient une **mémoire**, ce qui permet une règle nouvelle (« mot déjà joué ») qu'aucun calcul ne pouvait exprimer ; jour 3, tri et dichotomie, avec la démonstration que la dichotomie cesse de valoir dès que l'ordre change ; jour 4, le coup devient un objet et l'échange à trois têtes disparaît.

**Le fil conducteur du lot**, énoncé à l'élève au fil des quatre leçons : le désalignement de structures parallèles. Il apparaît à la leçon 7 (cinq structures à tenir d'accord), devient pénible à la leçon 10 (trois listes, trois retraits), franchement dangereux à la leçon 11 (un échange à trois têtes, qu'un oubli mélange définitivement et en silence), et disparaît à la leçon 12. La note de 12.3 le résume : « ce qu'un objet supprime, ce n'est pas du travail, c'est une possibilité de se tromper ». La question de réflexion de 12.3 demande d'ailleurs ce qui, dans le programme final, peut encore se désaligner — et renvoie à la leçon 13.

**Outillage.** `checkindice5.py` alignait à tort son filtre de déclarations sur une version élargie ; il reproduit maintenant **mot pour mot** celui du vérificateur (`using` n'en fait pas partie), ce qui a révélé une fuite qu'il laissait passer. `cslint.py` accepte désormais la dernière valeur d'un initialiseur de tableau sans virgule finale.
