...
(function () {
  ...
  var header = el("header", "day-header",
    '<div class="kicker">Projet full-stack — La Ludothèque (C# · ASP.NET Core · SQL Server) · 21 leçons</div>' +
  ...
  function updateScore() {
    ...
    try {
      var key = "projcsharp21-l" + DAY.num;
      var prev = parseInt(localStorage.getItem(key) || "0", 10);
      if (pts > prev) localStorage.setItem(key, String(pts));
      localStorage.setItem("projcsharp21-total-l" + DAY.num, String(totalQuestions));
