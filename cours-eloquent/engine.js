...
(function () {
  ...
  var header = el("header", "day-header",
    '<div class="kicker">Eloquent — la persistance de la Ludothèque · 21 leçons</div>' +
  ...
  function updateScore() {
    ...
    try {
      var key = "eloquent21-l" + DAY.num;
      var prev = parseInt(localStorage.getItem(key) || "0", 10);
      if (pts > prev) localStorage.setItem(key, String(pts));
      localStorage.setItem("eloquent21-total-l" + DAY.num, String(totalQuestions));
