/* ===== Moteur du cours Merise — rend la page à partir de l'objet DAY ===== */
(function () {
  "use strict";
  ...
  var header = el("header", "day-header",
    '<div class="kicker">Merise — Conception de bases de données · 21 leçons</div>' +
  ...
  wrap.appendChild(el("footer", "pagefoot", "Leçon " + DAY.num + " / 21 — Refais les exercices jusqu'à obtenir un score parfait. Et surtout, dessine chaque modèle sur papier : la modélisation s'apprend le crayon à la main !"));
  ...
  function balanceOpts(ex) {
    ...
    var tails = [
      ...
      ", surtout pour de très grandes bases de données",
      ", ce qui provoquerait plutôt une incohérence",
      ", au prix d'un modèle nettement plus lourd",
      ", ce qui mélange en réalité deux notions distinctes",
      ", bien que ce ne soit presque jamais nécessaire",
      ", à condition d'ajouter une contrainte supplémentaire",
      ", ce qui sort largement du cadre de cette leçon",
      ", en contournant les règles habituelles de la méthode",
      ", selon une approche aujourd'hui jugée obsolète",
      ", ce qui alourdirait inutilement le schéma",
      ", sauf si l'énoncé précise explicitement le contraire",
      ", une confusion fréquente chez les débutants",
      ", même si le résultat obtenu serait erroné",
      ", ce qui ne correspond pas à l'objectif recherché",
      ", en supposant à tort que le modèle est déjà validé",
