/* ===== Moteur du cours SQL — rend la page à partir de l'objet DAY ===== */
(function () {
  "use strict";
  ...
  var header = el("header", "day-header",
    '<div class="kicker">MySQL — Le SGBD open source le plus répandu · 21 leçons</div>' +
  ...
  wrap.appendChild(el("footer", "pagefoot", "Leçon " + DAY.num + " / 21 — Refais les exercices jusqu'à un score parfait. Et surtout, exécute chaque requête dans un vrai SGBD : le SQL s'apprend en interrogeant de vraies données !"));
  ...
  function balanceOpts(ex) {
    ...
    var tails = [
      ...
      ", surtout sur de très grandes tables",
      ", ce qui provoquerait plutôt une erreur SQL",
      ", au prix d'une requête nettement plus lourde",
      ", ce qui mélange en réalité deux notions distinctes",
      ", bien que ce ne soit presque jamais nécessaire",
      ", à condition d'installer une extension supplémentaire",
      ", ce qui sort largement du cadre de cette leçon",
      ", en contournant le fonctionnement normal du SGBD",
      ", selon une approche aujourd'hui jugée obsolète",
      ", ce qui ralentirait inutilement la requête",
      ", sauf si le schéma précise le contraire",
      ...
      ", en supposant à tort que les données sont déjà triées",
